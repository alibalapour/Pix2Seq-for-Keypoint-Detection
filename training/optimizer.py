...
def sam_function(model, img, seq_input, seq_expected, criterion, scaler, mixed_precision, sam_optimizer, grad_clip):
    enable_running_stats(model)

    with torch.cuda.amp.autocast(enabled=mixed_precision):
        preds_first = model(img, seq_input)
        loss = criterion(preds_first.reshape(-1, preds_first.shape[-1]), seq_expected.reshape(-1))
    ...
    with torch.cuda.amp.autocast(enabled=mixed_precision):
        outputs = model(img, seq_input)
        second_loss = loss = criterion(outputs.reshape(-1, outputs.shape[-1]), seq_expected.reshape(-1))
...
class GSAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, model, gsam_alpha, rho_scheduler, adaptive=False, perturb_eps=1e-12,
    ...
    @torch.no_grad()
    def set_closure(self, loss_fn, inputs, seq_input, seq_expected, mixed_precision, scaler, **kwargs):
        ...
        def get_grad():
            ...
            with torch.enable_grad():
                outputs = self.model(inputs, seq_input)
                loss = loss_fn(outputs.reshape(-1, outputs.shape[-1]), seq_expected.reshape(-1), **kwargs)
...
class GSAMLinearScheduler(SchedulerBase):
    def step_func(self):
...
def gsam_function(loss_fn, inputs, seq_input, seq_expected, scaler, mixed_precision, gsam_optimizer, grad_clip,
                  lr_scheduler):
    gsam_optimizer.set_closure(loss_fn, inputs, seq_input, seq_expected, mixed_precision, scaler)
